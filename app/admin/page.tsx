...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    // Realtime: actualiza automáticamente cuando cambia la tabla leads
  ...
  // ─── Cargar conversaciones del lead seleccionado ─────────────────────────
  ...
  useEffect(() => {
    ...
    // Realtime en conversaciones de este lead
    const channel = supabase
  ...
  async function toggleManual(lead: Lead) {
    ...
    await supabase
      .from('leads')
      .update({ modo_manual: newVal })
      .eq('id', lead.id)

    setLeads(prev => prev.map(l => l.id === lead.id ? { ...l, modo_manual: newVal } : l))
    if (selectedLead?.id === lead.id) {
      setSelectedLead(prev => prev ? { ...prev, modo_manual: newVal } : prev)
    }
  }
  ...
  async function saveNotes() {
    if (!selectedLead) return
    setSavingNotes(true)
    await supabase.from('leads').update({ notas: notes }).eq('id', selectedLead.id)
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ background: '#1a1a1a', border: '1px solid #333', borderRadius: 12, padding: 40, width: 340 }}>
          ...
          <form onSubmit={handleLogin}>
            ...
            <button type="submit" style={{ ...btnStyle, marginTop: 16, width: '100%', background: '#16a34a' }}>
              Entrar
            </button>
          </form>
        </div>
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ width: 320, borderRight: '1px solid #222', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '8px 0', flex: 1 }}>
          {leads.length === 0 && (
            <p style={{ color: '#555', fontSize: 13, padding: '16px', textAlign: 'center' }}>Sin leads todavía</p>
          )}
          {leads.map(lead => (
            <div
              key={lead.id}
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: 600, color: '#fff', fontSize: 14 }}>
                  {lead.listo_comprar ? '💰 ' : ''}{lead.nombre || 'Sin nombre'}
                </span>
                <span style={{ fontSize: 12, fontWeight: 700, color: scoreColor(lead.intent_score) }}>
                  {lead.intent_score}
                </span>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 4, alignItems: 'center' }}>
                <span style={{ background: estadoBadge(lead.estado), color: '#fff', fontSize: 10, padding: '2px 6px', borderRadius: 4 }}>
                  {lead.estado}
                </span>
                {lead.modo_manual && (
                  <span style={{ background: '#7c3aed', color: '#fff', fontSize: 10, padding: '2px 6px', borderRadius: 4 }}>manual</span>
                )}
          ...
          <div style={{ padding: '12px 20px', borderBottom: '1px solid #222', display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{
                  background: estadoBadge(selectedLead.estado),
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{
                  ...btnStyle,
                  background: selectedLead.modo_manual ? '#7c3aed' : '#374151',
                  fontSize: 12
                }}
              >
            ...
            {conversations.map(msg => (
              <div
                ...
                style={{
                  ...
                  background: msg.role === 'user' ? '#1f2937'
                    : msg.role === 'human' ? '#3b1f7c'
                    : '#1a2e1a',
                  borderRadius: 12,
                  padding: '10px 14px'
                }}
